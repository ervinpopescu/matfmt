...
cargo test           # run all tests (44 integration tests)
...
Settings can be stored in `matfmt.toml`. All fields optional, CLI flags override. Uses snake_case enum values (`all_functions`, `exclude_pow`, etc.).

## Architecture

Modules under `src/`:

- **`config.rs`** — `FormatterConfig` struct, enums (`IndentMode`, `OperatorSpacing`, `MatrixIndent`), and `FileConfig` for TOML loading. Enums derive both `clap::ValueEnum` and `serde::Deserialize`.
- **`indent.rs`** — `IndentEngine` processes each line for control structure indentation. Owns all control-structure regexes. Manages `istep`/`fstep` stacks and tracks block comments, line comments, ignore directives, and ellipsis line continuation.
...
- **`bin/matfmt.rs`** — CLI binary using `clap` derive. Loads config as: defaults <- matfmt.toml <- CLI flags.
...
- Config layering: `FormatterConfig::default()` <- `FileConfig` (from TOML) <- CLI args. `FileConfig` has all `Option` fields so only specified values override.
